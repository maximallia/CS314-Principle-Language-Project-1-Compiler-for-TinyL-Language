...
static int digit()
{
	...
	instrfield1 = next_register();
	CodeGen(LOADI, instrfield1, to_digit(token), EMPTY_FIELD);
	next_token();
	return instrfield1;
}
...
static int var()
{
  ...
  instrfield1 = next_register();
  CodeGen(LOAD, instrfield1, token, EMPTY_FIELD);
  next_token();
  return instrfield1;
...
static int arith_expr(){
  int instrfield1, instrfield2, instrfield3;
  switch (token){
  case '+':
    next_token();
    instrfield2 = expr();
    instrfield3 = expr();
    instrfield1 = next_register();
    CodeGen(ADD, instrfield1, instrfield2, instrfield3);
    return instrfield1;
    /* YOUR CODE GOES HERE */
  ...
  case'-':
    ...
    instrfield3 = expr();
    instrfield1 = next_register();
    CodeGen(SUB, instrfield1, instrfield2, instrfield3);
    return instrfield1;
  ...
  case '*':
    ...
    instrfield3 = expr();
    instrfield1 = next_register();
    CodeGen(MUL, instrfield1, instrfield2, instrfield3);
    return instrfield1;
 
  default:
    ERROR("Symbol %c unknown in arith_expr\n", token);
    exit(EXIT_FAILURE);
  }
}
...
static int logical_expr(){
  ...
  case '&':
    ...
    instrfield3 = expr();
    instrfield1 = next_register();
    CodeGen(AND, instrfield1, instrfield2, instrfield3);
    return instrfield1;
  ...
  case '|':
    ...
    instrfield3 = expr();
    instrfield1 = next_register();
    CodeGen(OR, instrfield1, instrfield2, instrfield3);
    return instrfield1;

  default:
    ERROR("Symbol %c unknown in logical_expr\n", token);
    exit(EXIT_FAILURE);
  }
}
...
static void assign()
{
  ...
  if(!is_identifier(token)){
    ...
    if(token == '='){
      next_token();
      instrfield2 = expr();
      CodeGen(STORE, var, instrfield2, EMPTY_FIELD);
    } 
  }
